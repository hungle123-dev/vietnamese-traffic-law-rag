...
- Exact lookup and Neo4j lexical retrieval plus independently implemented BKAI+PyVi dense retrieval, each with explicit offline index build; the retrieval-only API remains deferred.
- Evaluation runners for R0 and R1; R2 begins only after their dev reports are compared.
