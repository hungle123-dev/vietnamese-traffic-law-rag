...
- Neo4j full-text indexes and R0 evaluation reports are generated artifacts. `build-lexical-index` creates and waits offline for the fixed `Article`, `Clause`, and `Point` indexes; `search-lexical` and `evaluate-r0` first require all three to be `ONLINE` and reconcile their 6.343 indexed units with the frozen parsed snapshot. Generated reports remain outside Git.
